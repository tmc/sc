...
// 1. Shallow history mechanism to remember previous states
// 2. Transitions to history pseudostates
// 3. Default history behavior when no history exists
//
...
func HistoryStatechart() *semantics.Statechart {
	return semantics.NewStatechart(&sc.Statechart{
		...
		Transitions: []*sc.Transition{
			...
			// CloseSettings returns to history - remembers last Active substate
			{
				Label: "CloseSettings",
...
// HistoryStatechartDeep creates a statechart that demonstrates deep history semantics.
// Similar to HistoryStatechart but uses deep history (H*) instead of shallow history.
func HistoryStatechartDeep() *semantics.Statechart {
	return semantics.NewStatechart(&sc.Statechart{
		RootState: &sc.State{
			Label: "TextEditor",
			Type:  sc.StateTypeNormal,
		...
		Transitions: []*sc.Transition{
			...
			// CloseSettings returns to deep history - remembers full nested configuration
			{
				Label: "CloseSettings",
				From:  []string{"Settings"},
				To:    []string{"H_star"},
