...
func NewLeakDetector() *LeakDetector {
	...
	return &LeakDetector{
		...
		cancel:        cancel,
		thresholds: LeakThresholds{
			MemoryGrowthRate:    512 * 1024, // 512KB/sec
			ObjectGrowthRate:    5000,       // 5k objects/sec
			GoroutineGrowthRate: 10,         // 10 goroutines/sec
			MinSampleDuration:   1 * time.Second,
			MinGrowthAmount:     1 * 1024 * 1024, // 1MB
		},
...
type StatechartLeakDetector struct {
	*LeakDetector
	...
	thresholds     StatechartLeakThresholds
	mu             sync.RWMutex
}
...
func (sld *StatechartLeakDetector) analyzeMachine(id string, machine *semantics.MachineWrapper) StatechartMachineLeakReport {
	...
	sld.mu.RLock()
	thresholds := sld.thresholds
	previous := sld.machineMetrics[id]
	sld.mu.RUnlock()
	...
	if len(history) >= thresholds.MaxHistorySize {
		report.Issues = append(report.Issues, MachineIssue{
			Type:        HistoryLeak,
	...
	if len(errors) >= thresholds.MaxErrorCount {
		report.Issues = append(report.Issues, MachineIssue{
			Type:        MemoryLeak,
	...
	if len(context.Fields) >= thresholds.MaxContextSize {
		report.Issues = append(report.Issues, MachineIssue{
	...
	// Check for abnormal growth rate in machine history.
	if !previous.LastActive.IsZero() {
		elapsed := report.Metrics.LastActive.Sub(previous.LastActive).Seconds()
		if elapsed > 0 {
			growthRate := float64(report.Metrics.HistorySize-previous.HistorySize) / elapsed
			if growthRate >= thresholds.GrowthRateThreshold {
				report.Issues = append(report.Issues, MachineIssue{
					Type:        HistoryLeak,
					Description: fmt.Sprintf("rapid history growth: %.2f steps/sec", growthRate),
					Severity:    Medium,
					Value:       growthRate,
				})
			}
	...
	sld.mu.Lock()
	sld.machineMetrics[id] = report.Metrics
	sld.mu.Unlock()
