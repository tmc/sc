...
extension FlowView {
    ...
    func drawGrid(context: GraphicsContext, size: CGSize) {
        ...
        let path = Path { path in
            ...
            let totalOffsetX = offset.width + drawing_currentDragOffset.width
            let totalOffsetY = offset.height + drawing_currentDragOffset.height
        ...
        // Draw Origin Cross
        let originX = size.width / 2 + (offset.width + drawing_currentDragOffset.width)
        let originY = size.height / 2 + (offset.height + drawing_currentDragOffset.height)
    ...
    func drawNode(context: GraphicsContext, node: FlowNode, phase: TimeInterval) {
        ...
        let isHovered = (drawing_hoveredNodeID == node.id)
        ...
        if drawing_hoveredParentID == node.id {
        ...
        if node.type == .compound {
            ...
            let headerPath = Path { p in
                p.move(to: CGPoint(x: nodeRect.minX, y: nodeRect.minY + 12)) // Start below corner
                p.addRoundedRect(in: nodeRect, cornerSize: CGSize(width: 12, height: 12))
            }
            // Just draw line
        ...
        // 7. Connection Ports
        if (drawing_connectingEdge != nil || isSelected) && scale > 0.4 {
    ...
    func calculateConnectionPoints(from sourceSpy: CGRect, to targetSpy: CGRect) -> (CGPoint, CGPoint) {
    ...
    func intersectionPoint(rect: CGRect, center: CGPoint, other: CGPoint) -> CGPoint {
